...
description: >-
  ...
  edges. Manual invoke only (/gs-to-tickets).
...
2. 執行 **缺 setup 閘門**：無 `AW/issue-tracker.md`（`AW` 見 `docs/agent-work-and-setup.md`）則提示 **`/gs-setup`**（或「幫我 setup」），完成前不要寫 tickets。
...
- **Local（agent-work）**：`AW/<feature>/issues/<NN>-<slug>.md`，從 `01`、依賴先寫；一票一檔。
...
```
### 下一步建議
建議執行 `/gs-implement`：依核准的 tickets／spec 實作（內含 TDD，收尾 code-review）。
```
